...
class DPAL(nn.Module):
    ...
    def __init__(self, model, args, optimizer, ad_optimizer, steps=1, episodic=False, margin_e0=1.0 * math.log(1000),
        ...
        self.model_state, self.optimizer_state, self.ad_optimizer_state = copy_model_and_optimizer(self.model,
                                                                                                   self.optimizer,
                                                                                                   self.ad_optimizer)
    ...
    def forward(self, x):
        ...
        for _ in range(self.steps):
            ...
            outputs, ema, reset_flag = self.forward_and_adapt_dpal(x, self.model, self.optimizer, self.ad_optimizer,
    ...
    def get_attention_weights(self, module, input, output):
        if self.attention_weights is None:
            ...
            self.attention_weights = torch.cat((self.attention_weights, output.mean(0).mean(0)[0, 2:].unsqueeze(0)),
                                               dim=0)
    ...
    def get_attention_weights_before_softmax(self, module, input, output):
        ...
        if self.attention_weights_before_softmax is None:
            ...
            self.attention_weights_before_softmax = torch.cat(
                (self.attention_weights_before_softmax, attn.mean(0).mean(0)[0, 2:].unsqueeze(0)), dim=0)
    ...
    def forward_and_adapt_dpal(self, x, model, optimizer, ad_optimizer, margin, reset_constant, ema):
        ...
        similarity_loss = torch.stack(
            [-nn.functional.cosine_similarity(noise.unsqueeze(1), noise.unsqueeze(0), dim=2).mean() for noise in
             noises_output]).mean()
    ...
    def forward_and_adapt_tent(self, x, model, optimizer):
        ...
        outputs = model(x)
        # adapt
        loss = self.softmax_entropy(outputs).mean(0) - self.args.lambda_entropy * self.entropy(
            self.attention_weights).mean()
        loss.backward()
        optimizer.step()
        optimizer.zero_grad()
        return outputs
